...
Ratio = [Ratio1(:,1:40), Ratio2(:,41:70)];
...
%Saline
yS = xlsread('PerfTrialSalH1.xlsx'); ySmean = mean(yS);
y2S = xlsread('RatioSalH1.xlsx');    y2Smean = mean(y2S);
y3S = xlsread('NactsSalH1.xlsx');    y3Smean = mean(y3S);
...
clf
figure(1)
plot(1:40, yLDmean(1:40), '-r','LineWidth',2)
shadedErrorBar(1:40,yLD(:,1:40),{@mean,@std},'lineprops','-r', 'patchSaturation',0.13)
ylabel('Proportion of perfect trials')
xlabel('Sessions')
set(gca,'fontsize',14)
yline(0.4, 'k--')
hold on 
plot(1:40, ySmean(1:40), '-b','LineWidth',2)
box off
shadedErrorBar(1:40,yS(:,1:40),{@mean,@std},'lineprops','-b', 'patchSaturation',0.13)
ylim([0 1])
xlim([0,40])
